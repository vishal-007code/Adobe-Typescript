...
export class AdobePage {
  ...
  constructor(page: Page) {
    ...
    this.letsGoIndicator = page.getByText(
      "Hold tight. We’re loading your school account.",
      { exact: true }
    );
  }
  ...
  async getLoginProvider(): Promise<string> {
    const supportedProviderPattern =
      /^https:\/\/(?:accounts\.google\.com|login\.microsoftonline\.com)\//i;
    ...
    if (!supportedProviderPattern.test(providerUrl)) {
      throw new Error(
        `Adobe login did not redirect to a supported provider. Current URL: ${providerUrl}`
      );
    }
